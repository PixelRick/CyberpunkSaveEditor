...
public:
    const std::vector<CName>& subsys_names() const {
        return m_subsys_names;
    }
    std::vector<CName>& subsys_names() {
        return m_subsys_names;
    }

    const std::vector<CObjectSPtr>& objects() const {
        return m_objects;
    }
    std::vector<CObjectSPtr>& objects() {
        return m_objects;
    }
...
public:
    ...
    bool serialize_in_sized(
        std::istream&       reader,
        redx::csav::version version,
        uint32_t            blob_size,
        bool                do_cnames) {
        ...
        for (auto it = obj_descs.begin(); it != obj_descs.end(); ++it) {
            ...
            auto new_obj =
                std::make_shared<CObject>(obj_ctypename, false); // todo: static create method
        ...
        m_handle_objects.assign(
            serobjs.begin() + root_obj_cnt, serobjs.end()); // backup handle-objects
    ...
    bool serialize_out(std::ostream& writer) const {
        ...
        serctx.m_objects.insert(
            serctx.m_objects.end(), m_handle_objects.begin(), m_handle_objects.end());
        ...
        serctx.strpool.serialize_out(
            stw, base_spos, new_header.strpool_data_offset, strpool_data_size);
        ...
        auto objdata =
            ss.str(); // not optimal but didn't want to depend on boost and no time to dev a mem stream for now
