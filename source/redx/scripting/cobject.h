...
class CObject : public std::enable_shared_from_this<const CObject>, public CPropertyOwner {
private:
    struct Field {
        Field() = default;

        Field(gname name, CPropertyUPtr&& prop)
            : name(name)
            , prop(std::move(prop)) {
        }
...
protected:
    ...
    [[nodiscard]] bool serialize_field(
        Field&         field,
        std::istream&  is,
        CSystemSerCtx& serctx,
        bool           eof_is_end_of_prop = false) {
        if (!is.good())
            return false;
...
public:
    ...
    [[nodiscard]] bool
    serialize_in(std::istream& is, CSystemSerCtx& serctx, bool eof_is_end_of_object = false) {
        ...
        for (size_t i = 0, n = serial_descs.size(); i < n; ++i) {
            ...
            fdesc = {strpool.at(sdesc.name_idx).gstr(), strpool.at(sdesc.ctypename_idx).gstr()};
        ...
        serctx.log(fmt::format(
            "serialized_in CObject {} in {} bytes",
            this->ctypename().c_str(),
            (size_t)(end_pos - start_pos)));

        post_cobject_event(ObjectEventType::data_modified);
        return true;
    }
    ...
    [[nodiscard]] bool serialize_out(std::ostream& os, CSystemSerCtx& serctx) const {
        ...
        for (auto& field : m_fields) {
            ...
            descs.emplace_back(name_idx, ctypename_idx, data_offset);

            if (!field.prop->serialize_out(os, serctx)) {
                serctx.log(fmt::format(
                    "couldn't serialize_out {}::{}",
                    this->ctypename().c_str(),
                    field.name.c_str()));
                return false;
            }
            ...
            serctx.log(fmt::format(
                "serialized_out {}::{} in {} bytes",
                this->ctypename().c_str(),
                field.name.c_str(),
                prop_end_pos - prop_start_pos));
        }
        ...
        serctx.log(fmt::format(
            "serialized_out CObject {} in {} bytes",
            this->ctypename().c_str(),
            (size_t)(end_pos - start_pos)));
    ...
    static std::string_view field_name_getter(const Field& field) {
    ...
    [[nodiscard]] bool imgui_widget(const char* label, bool editable) {
        ...
        if (ctypename() == gn_WorldPosition) {
            return imgui_widget_wpos(label, editable);
        }

        if (ctypename() == gn_Quaternion) {
        ...
        if (ImGui::BeginTable(label, show_field_types ? 3 : 2, tbl_flags)) {
            ...
            for (auto& field : m_fields) {
                ...
                if (show_field_types) {
                    auto Fieldype = field.prop->ctypename();
                    ImGui::Text(Fieldype.c_str());
                ...
                if (!prop->imgui_is_one_liner()) {
                    ...
                    if (prop->ctypename() != gn_WorldPosition && prop->ctypename() != gn_Quaternion)
...
protected:
    std::set<ObjectListener*> m_listeners;

    void post_cobject_event(ObjectEventType evt) const {
        std::set<ObjectListener*> listeners = m_listeners;
...
public:
    ...
    void add_listener(ObjectListener* listener) const {
        auto& listeners = const_cast<CObject*>(this)->m_listeners;
        listeners.insert(listener);
    }

    void remove_listener(ObjectListener* listener) const {
        auto& listeners = const_cast<CObject*>(this)->m_listeners;
