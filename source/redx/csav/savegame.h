...
public:
    ...
    op_status open_with_progress(
        std::filesystem::path path,
        progress_t&           progress,
        bool                  dump_decompressed_data = false,
        ...
        try_load_node_data_struct(
            chtrcustom, "CharacetrCustomization_Appearances", progress, 0.35f, test);
    ...
    bool reload_character_customization() {
        progress_t dummy;
        return try_load_node_data_struct(
            chtrcustom, "CharacetrCustomization_Appearances", dummy, 0.f, true);
    }

    op_status save_with_progress(
        std::filesystem::path path,
        progress_t&           progress,
        bool                  dump_decompressed_data = false) {
        progress.value = 0.00f;

        try_save_node_data_struct(inventory, "inventory");
        progress.value = 0.10f;
        try_save_node_data_struct(chtrcustom, "CharacetrCustomization_Appearances");
        progress.value = 0.15f;

        try_save_node_data_struct(godmode, "godModeSystem");
        progress.value = 0.20f;
        try_save_node_data_struct(factsdb, "FactsDB");
        progress.value = 0.25f;

        try_save_node_data_struct(scriptables, "ScriptableSystemsContainer");
        progress.value = 0.30f;
        try_save_node_data_struct(psdata, "PSData");
        progress.value = 0.60f;

        try_save_node_data_struct(stats, "StatsSystem");
        progress.value = 0.70f;
        try_save_node_data_struct(statspool, "StatPoolsSystem");
        progress.value = 0.80f;
    ...
    search_node(const shared_node_type& node, std::string_view name) const {
        if (node->name() == name)
            return node;
...
protected:
    bool try_load_node_data_struct(
        redx::csav::node_serializable& var,
        ...
        if (load_node_data_struct(node, var)) {
            if (test) {
                ...
                progress.comment.assign(
                    fmt::format("Testing reserialization of node_t {}", node->name()));
    ...
    bool test_reserialize(const shared_node_type& node, redx::csav::node_serializable& var) {
        ...
        if (stree1.nodedata.size() != stree2.nodedata.size()
            || std::memcmp(
                stree1.nodedata.data(), stree2.nodedata.data(), stree1.nodedata.size())) {
            std::ofstream ofs;
    ...
    bool load_node_data_struct(const shared_node_type& node, node_serializable& var) {
        try {
            ...
            MessageBoxA(
                0,
                fmt::format("couldn't load node_t {}\nreason: {}", node->name(), e.what()).c_str(),
                "error",
                0);
            return false;
        }
