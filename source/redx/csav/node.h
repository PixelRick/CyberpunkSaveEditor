...
class node_t : public std::enable_shared_from_this<const node_t>, public node_listener_t {
    struct create_tag {};
...
public:
    ...
    const std::vector<std::shared_ptr<const node_t>>& children() const {
        return m_children;
    }
    ...
    bool is_root() const {
        return m_idx == root_node_idx;
    }
    bool is_blob() const {
        return m_idx == blob_node_idx;
    }
    bool is_cnode() const {
        return m_idx >= 0;
    }
    ...
    size_t calcsize() const {
        ...
        return std::accumulate(
            m_children.begin(), m_children.end(), base_size, [](size_t cnt, auto& node) {
                return cnt + node->calcsize();
            });
    }
...
class node_reader : public std::istream {
...
class node_writer : public std::ostringstream {
...
private:
    virtual bool
    from_node_impl(const std::shared_ptr<const node_t>& node, const version& version) = 0;
