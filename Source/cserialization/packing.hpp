...
int64_t read_packed_int(IStream& is)
{
  char a = 0;
  is.read(&a, 1);
...
void write_packed_int(OStream& os, int64_t value)
{
  uint8_t packed[5] = {};
  uint8_t cnt = 1;
...
template <typename IStream>
std::string read_str(IStream& is)
{
...
template <typename OStream>
void write_str(OStream& os, const std::string& s)
{
