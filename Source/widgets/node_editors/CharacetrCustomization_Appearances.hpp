...
class CharacetrCustomization_Appearances_editor
  : public node_editor_widget
{
  CCharacterCustomization m_customization;

public:
  CharacetrCustomization_Appearances_editor(const std::shared_ptr<const node_t>& node, const csav_version& version)
    : node_editor_widget(node, version)
  {
    reload();
  }

  ~CharacetrCustomization_Appearances_editor() override {}

public:
  bool commit_impl() override
  {
    auto rebuilt = m_customization.to_node(version());
    auto curnode = ncnode();
  ...
  {
    bool success = m_customization.from_node(node(), version());
    return success;
  }
...
protected:
  bool draw_impl(const ImVec2& size) override
  {
    ...
    return CCharacterCustomization_widget::draw(m_customization);;
  }
};
